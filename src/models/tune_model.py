...
"""
modelfit function - Gets the number of boosting rounds 
using XGBoost Cross Validation Method

xgb_gridsearch function - Get the best parameters of the XGBoost model 
using Pipeline, GridSearchCV and DataProcessor class
...
def modelfit(xgb_reg, X_train, y_train, X_test, y_test):
    """
    Get the number of boosting rounds of the model 
    using XGBoost Cross Validation Method
    
    Use the above parameters to Fit the XGBoost model on the train 
    and get feature importance and predictons
...
def xgb_gridsearch(param_test, xgb_reg):
    """
    Get the best parameters of the XGBoost model using Pipeline, GridSearchCV 
    and DataProcessor class
    ...
    pipeline = Pipeline([("dataprocessor", dp), ("xgb_reg", xgb_reg)])
